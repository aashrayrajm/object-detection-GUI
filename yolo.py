...
layout = 	[
		[sg.Text('YOLO')],
		[sg.Text('Path to image'), sg.In(r'C:/Users/Aashray Raj Mathur/Desktop/PySimpleGUI-YOLO/images/baggage_claim.jpg',size=(40,1), key='image'), sg.FileBrowse()],
...
def counting_each_object(labels):
...
if len(idxs) > 0:
	...
	for index, i in enumerate(idxs.flatten(), start=1):
		...
		count = index
	object_count = "Total No Of Objects: "+str(count)
...
image = cv2.putText(image, object_count, (10, offset),
...
root = tk.Tk()
img = tk.PhotoImage(file="output_files/output.png")
photo = tk.Label(root, image=img).pack()
lab = tk.Label(root, text=object_count).pack()
tk.Label(root, text=str(counting_each_object(labels))).pack()
